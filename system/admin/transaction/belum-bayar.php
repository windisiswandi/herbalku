...
$select_invoice = $server->query("SELECT * FROM `akun`, `invoice` WHERE invoice.tipe_progress='Belum Bayar' AND invoice.id_user=akun.id ORDER BY `invoice`.`idinvoice` DESC");
            ...
            <div class="isi_cart" id="isi_cart<?php echo $invoice_data['id']; ?>">
                <div class="box_gambar_judul" onclick="show_detail_invoice('<?php echo $url; ?>assets/image/product/<?php echo $exp_gambar_od[0]; ?>', `<?php echo $item['judul']; ?>`, '<?php echo $item['nama']; ?>', '<?php echo $item['qty']; ?>', '#<?php echo $invoice_data['idinvoice']; ?>', '<?php echo $invoice_data['tipe_progress']; ?>', 'Sudah Dibayar', '<?php echo $invoice_data['nama_lengkap']; ?>', '<?php echo $exp_prov_od[1] . ', ' . $exp_kota_od[1] . ', ' . $invoice_data['alamat_lengkap']; ?>', '<?php echo strtoupper($invoice_data['kurir']) . ' ' . $invoice_data['layanan_kurir']; ?>', 'Rp <?php echo number_format($invoice_data['total_harga'], 0, '.', '.'); ?>', '<?php echo $url . 'admin/transaction/print/invoice/' . $invoice_data['idinvoice']; ?>')">
                    ...
                    <div class="box_judul_ic">
                        ...
                        <?php
                        if ($invoice_data['provinsi'] || $invoice_data['kota'] || $invoice_data['alamat_lengkap']) {
                        ?>
                            <p><?php echo $exp_prov_od[1] . ', ' . $exp_kota_od[1] . ', ' . $invoice_data['alamat_lengkap']; ?></p>
                        <?php
                        }
                        if ($invoice_data['bank_manual'] || $invoice_data['bukti_transfer']) {
                        ?>
                            <a href="<?php echo $url; ?>assets/image/bukti-transfer/<?php echo $invoice_data['bukti_transfer']; ?>" target="_blank" class="lbt_bb">Lihat Bukti Transaksi</a>
                        <?php
                        }
                        ?>
                    </div>
                ...
                <div class="box_detail_isi_cart">
                    ...
                    if ($invoice_data['bank_manual'] || $invoice_data['bukti_transfer']) {
                    ?>
                        <div class="bayar" onclick="step_paket_konfirmasi('<?php echo $invoice_data['idinvoice']; ?>', '<?php echo $invoice_data['id_user']; ?>')">
                            <p id="text_spk<?php echo $invoice_data['idinvoice']; ?>">Konfirmasi</p>
                            <img src="<?php echo $url; ?>assets/icons/loading-w.svg" class="loading_spk" id="load_spk<?php echo $invoice_data['idinvoice']; ?>">
                        </div>
                    <?php
                    }
                    ?>
