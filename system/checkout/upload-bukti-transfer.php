...
$update_bt = $server->query("UPDATE `invoice` SET `bank_manual`='$nama_bank_bt',`bukti_transfer`='$name1',`waktu_transaksi`='$time',`kode_unik`=$kode_unik WHERE `idinvoice`='$id_inv_bt' ");

$select_user_mp = $server->query("SELECT * FROM `invoice`, `invoice_item`,`iklan`, `akun` WHERE invoice.idinvoice='$id_inv_bt' AND invoice.idinvoice=invoice_item.idinvoice AND invoice_item.id_iklan=iklan.id and invoice.id_user=akun.id ");
